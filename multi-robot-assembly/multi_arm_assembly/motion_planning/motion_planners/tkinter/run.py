...
from ..prm import prm
from ..rrt import rrt
from ..rrt_connect import birrt, rrt_connect
from ..rrt_star import rrt_star
from ..smoothing import smooth_path
from ..trajectory.discretize import A_MAX, V_MAX, distance_discretize_curve
from ..trajectory.limits import analyze_continuity
from ..trajectory.linear import solve_multi_linear
from ..trajectory.smooth import get_curve_collision_fn, plot_curve, smooth_curve
from ..utils import (
    INF,
    compute_path_cost,
    elapsed_time,
...
def buffer_durations(durations, switch_t=0.0, min_t=0.0):
...
def retime_path(path, collision_fn=lambda q: False, smooth=False, **kwargs):
    ...
    print(
        "Velocity: t={:.3f}, error={:.3E}".format(
            *analyze_continuity(positions_curve.derivative(nu=1))
        )
    )
    print(
        "Acceleration: t={:.3f}, error={:.3E}".format(
            *analyze_continuity(positions_curve.derivative(nu=2))
        )
    )
    ...
    curve_collision_fn = get_curve_collision_fn(
        collision_fn, max_velocities=v_max, max_accelerations=a_max
    )
    positions_curve = smooth_curve(
        positions_curve,
        # v_max=None, a_max=None,
        v_max=v_max,
        a_max=a_max,
        curve_collision_fn=curve_collision_fn,
        **kwargs,
    )
    print("Position: t={:.3f}, error={:.3E}".format(*analyze_continuity(positions_curve)))
    print(
...
def problem1():
    ...
    obstacles = [
        create_box(center=(0.35, 0.75), extents=(0.25, 0.25)),
        # create_box(center=(.75, .35), extents=(.25, .25)),
        create_box(center=(0.75, 0.35), extents=(0.22, 0.22)),
        create_box(center=(0.5, 0.5), extents=(0.25, 0.25)),
        # create_box(center=(.5, .5), extents=(.22, .22)),
        create_cylinder(center=(0.25, 0.25), radius=0.1),
    ]

    # TODO: alternate sampling from a mix of regions
    regions = {
        "env": create_box(center=(0.5, 0.5), extents=(1.0, 1.0)),
        "green": create_box(center=(0.8, 0.8), extents=(0.1, 0.1)),
    }
    # start = np.array([0., 0.])
...
def infeasible():
    obstacles = [
        create_box(center=(0.25, 0.5), extents=(0.5, 0.05)),
        create_box(center=(0.5, 0.25), extents=(0.05, 0.5)),
    ]
...
def solve_lazy_prm(viewer, start, goal, sample_fn, extend_fn, collision_fn, radius=4, **kwargs):
    path, samples, edges, colliding_vertices, colliding_edges = lazy_prm(
        start, goal, sample_fn, extend_fn, collision_fn, **kwargs
    )
    ...
    for v1, v2 in edges:
        if (colliding_vertices.get(v1, False) is True) or (
            colliding_vertices.get(v2, False) is True
        ):
            colliding_edges[v1, v2] = True

    red_edges = [
        (samples[v1], samples[v2]) for (v1, v2), c in colliding_edges.items() if c is True
    ]
    green_edges = [
        (samples[v1], samples[v2]) for (v1, v2), c in colliding_edges.items() if c is False
    ]
    blue_edges = [
        (samples[v1], samples[v2]) for v1, v2 in edges if (v1, v2) not in colliding_edges
    ]
    add_roadmap(viewer, red_edges, color="red")
    add_roadmap(viewer, green_edges, color="green")
    add_roadmap(viewer, blue_edges, color="blue")
    print(
        "Edges | Colliding: {}/{} | CFree: {}/{} | Unchecked: {}/{}".format(
            len(red_edges),
            len(edges),
            len(green_edges),
            len(edges),
    ...
    print(
        "Vertices | Colliding: {}/{} | CFree: {}/{} | Unchecked: {}/{}".format(
            len(red_vertices),
            len(samples),
            len(green_vertices),
            len(samples),
...
def main(draw=True):
    ...
    parser.add_argument("-a", "--algorithm", default="rrt_connect", help="The algorithm to use.")
    parser.add_argument(
        "--anytime", action="store_true", help="Run the planner in an anytime mode."
    )
    parser.add_argument(
        "-d", "--draw", action="store_true", help="When enabled, draws the roadmap"
    )
    parser.add_argument("-r", "--restarts", default=0, type=int, help="The number of restarts.")
    parser.add_argument("-s", "--smooth", action="store_true", help="When enabled, smooths paths.")
    parser.add_argument("-t", "--time", default=1.0, type=float, help="The maximum runtime.")
    ...
    with profiler(field="tottime"):  # cumtime | tottime
        ...
        for _ in range(args.restarts + 1):
            ...
            collision_fn, colliding, cfree = wrap_collision_fn(
                get_collision_fn(environment, obstacles)
            )
            sample_fn, samples = wrap_sample_fn(
                get_sample_fn(environment, obstacles=[], use_halton=True)
            )  # obstacles
            ...
            print(
                "Solutions ({}): {} | Colliding: {} | CFree: {} | Time: {:.3f}".format(
                    len(paths),
                    [(len(path), round(compute_path_cost(path, cost_fn), 3)) for path in paths],
                    len(colliding),
                    len(cfree),
            ...
            for i, path in enumerate(paths):
                ...
                print(
                    "{}) Length: {} | Cost: {:.3f} | Ratio: {:.3f}".format(
                        i, len(path), cost, cost / min_distance
                    )
                )
                # path = path[:1] + path[-2:]
            ...
            if False and args.smooth:
                for path in paths:
                    ...
                    print(
                        "Smoothed distance_fn: {:.3f}".format(
                            compute_path_cost(smoothed, distance_fn)
                        )
                    )
