...
from .viewer import STEP_SIZE, contains, is_collision_free, point_collides, sample_line
...
def get_connected_test(obstacles, max_distance=0.25):  # 0.25 | 0.2 | 0.25 | 0.5 | 1.0
    ...
    def connected_test(q1, q2):
        ...
        are_connected = (get_distance(q1, q2) <= threshold) and is_collision_free(
            (q1, q2), obstacles
        )
        if are_connected:
...
def get_threshold_fn(d=2):
    ...
    threshold_fn = lambda n: gamma * (math.log(n) / n) ** (1.0 / d)
    return threshold_fn
...
def get_extend_fn(circular={}, step_size=STEP_SIZE, norm=INF):
    ...
    def fn(q1, q2):
        ...
        for i in range(num_steps):
            q = (1.0 / (num_steps - i)) * np.array(difference_fn(q2, q)) + q
