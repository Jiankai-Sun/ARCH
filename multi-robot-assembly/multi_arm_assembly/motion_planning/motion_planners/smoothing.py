    ...
    **kwargs,
    ...
    for iteration in irange(max_iterations):
        ...
        if verbose:
            ...
            print(
                "Iteration: {} | Waypoints: {} | Cost: {:.3f} | Time: {:.3f}".format(
        ...
        if (len(shortcut) < (j - i)) and all(
            not collision_fn(q) for q in default_selector(shortcut)
        ):
...
def smooth_path(
    ...
    for iteration in irange(max_iterations):
        if (
            ...
            or (elapsed_time(last_time) > converge_time)
            or (len(waypoints) <= 2)
        ):
            break
        ...
        if verbose and (print_frequency is not None) and (iteration % print_frequency == 0):
            ...
            print(
                "Iteration: {} | Waypoints: {} | Cost: {:.3f} | Elapsed: {:.3f} | Remaining: {:.3f}".format(
                    iteration,
                    len(waypoints),
                    cost,
        ...
        if new_cost >= cost:  # TODO: cost must have percent improvement above a threshold
            continue
        if not any(
            collision_fn(q) for q in default_selector(refine_waypoints(shortcut, extend_fn))
        ):
