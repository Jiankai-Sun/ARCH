...
from multi_arm_assembly.motion_planning.motion_planners.utils import (
    INF,
    compute_path_cost,
    default_selector,
    elapsed_time,
...
from .primitives import default_weights, get_distance_fn, get_embed_fn

Metric = namedtuple("Metric", ["p_norm", "weights"])
Node = namedtuple("Node", ["g", "parent"])
Solution = namedtuple(
    "PRMSolution", ["path", "samples", "edges", "colliding_vertices", "colliding_edges"]
)
...
class Roadmap(object):
    ...
    def add_samples(self, samples):
        ...
        for v1, sample in self.nearest.add_data(samples):
            ...
            for d, v2, _ in self.nearest.query_neighbors(
                sample,
                k=max_degree,
                eps=self.approximate_eps,
                p=self.p_norm,
    ...
    def is_colliding(self, v1, v2):
        edge = (v1, v2)
        return (
            self.colliding_vertices.get(v1, False)
            or self.colliding_vertices.get(v2, False)
            or self.colliding_edges.get(edge, False)
        )
...
def get_metrics(conf, weights=None, p_norm=2, distance_fn=None, cost_fn=None):
    ...
    if cost_fn is None:
        if distance_fn is None:
            cost_fn = get_distance_fn(
                weights, p_norm=p_norm
            )  # TODO: additive cost, acceleration cost
        else:
            cost_fn = distance_fn
...
def lazy_prm(
    ...
    **kwargs,
    ...
    start_time = time.time()
    weights, distance_fn, cost_fn = get_metrics(
        start, weights=weights, p_norm=p_norm, distance_fn=distance_fn, cost_fn=cost_fn
    )
    if roadmap is None:
        roadmap = Roadmap(
            extend_fn,
    ...
    while (elapsed_time(start_time) < max_time) and (path is None):  # TODO: max_attempts
        ...
        if verbose:
            print(
                "Candidate | Length: {} | Cost: {:.3f} | Vertices: {} | Samples: {} | Degree: {:.3f} | Time: {:.3f}".format(
                    len(lazy_path),
                    compute_path_cost(lazy_path, cost_fn=weight_fn),
                    len(roadmap.colliding_vertices),
                    len(roadmap.colliding_intermediates),
                    degree,
    ...
    if path is None:
        ...
        if verbose:
            print(
                "Failure | Forward: {} | Backward: {} | Vertices: {} | Samples: {} | Degree: {:.3f} | Time: {:.3f}".format(
                    len(forward_visited),
                    len(backward_visited),
                    len(roadmap.colliding_vertices),
                    len(roadmap.colliding_intermediates),
                    degree,
                    elapsed_time(start_time),
    ...
    if verbose:
        print(
            "Solution | Length: {} | Cost: {:.3f} | Vertices: {} | Samples: {} | Degree: {:.3f} | Time: {:.3f}".format(
                len(path),
                compute_path_cost(path, cost_fn=weight_fn),
                len(roadmap.colliding_vertices),
                len(roadmap.colliding_intermediates),
                degree,
                elapsed_time(start_time),
...
def create_param_sequence(initial_samples=100, step_samples=100, **kwargs):
    ...
    return (
        merge_dicts(kwargs, {"num_samples": num_samples})
...
def lazy_prm_star(
    ...
    **kwargs,
    ...
    for i, params in enumerate(param_sequence):
        ...
        if verbose:
            print(
                "Path: {} | Cost: {:.3f} | Length: {}".format(
                    is_path(new_path), new_cost, get_length(new_path)
                )
            )
