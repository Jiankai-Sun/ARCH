...
from ..utils import INF, even_space, get_distance
from .limits import find_max_acceleration, find_max_velocity
from .retime import get_interval, spline_duration

V_MAX = 0.8 * np.ones(2)
A_MAX = abs(V_MAX - 0.0) / abs(0.2 - 0.0)
# V_MAX = INF*np.ones(2)
# A_MAX = 1e6*np.ones(2)
...
def time_discretize_curve(
    ...
    **kwargs,
    ...
    if max_velocities is None:
        ...
        v_max_t, max_v = find_max_velocity(
            positions_curve, start_t=start_t, end_t=end_t, norm=norm
        )
        a_max_t, max_a = find_max_acceleration(
            positions_curve, start_t=start_t, end_t=end_t, norm=norm
        )
        ...
        if verbose:
            print(
                "Max velocity: {:.3f}/{:.3f} (at time {:.3f}) | Max accel: {:.3f}/{:.3f} (at time {:.3f}) | "
                "Step: {:.3f} | Duration: {:.3f}".format(
                    max_v,
                    np.linalg.norm(V_MAX, ord=norm),
                    v_max_t,
                    max_a,
                    np.linalg.norm(A_MAX, ord=norm),
                    a_max_t,
                    time_step,
                    spline_duration(positions_curve),
                )
            )  # 2 | INF
    else:
...
def derivative_discretize_curve(
    positions_curve, resolution=DEFAULT_RESOLUTION, time_step=1e-3, **kwargs
):
