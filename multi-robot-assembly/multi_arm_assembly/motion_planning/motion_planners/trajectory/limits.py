        ...
        for k in range(spline.c.shape[-1]):
            ...
            if v_max is not None:
                ...
                if any(abs(vel_poly(t)) > v_max[k] for t in boundary_ts):
                    return False
                if any(
                    not isinstance(r, complex) and (t0 <= r <= t1)
                    for s in signs
                    for r in (vel_poly + s * np.poly1d([v_max[k]])).roots
                ):
                    return False
            # a_max = None
            ...
            if a_max is not None:  # INF
                ...
                if any(
                    not isinstance(r, complex) and (t0 <= r <= t1)
                    for s in signs
                    for r in (accel_poly + s * np.poly1d([a_max[k]])).roots
                ):
                    return False
    return True
...
def check_spline(spline, v_max=None, a_max=None, verbose=False, **kwargs):
    ...
    if v_max is not None:
        ...
        if verbose:  # or violation:
            print(
                "Violation: {} | Max velocity: {:.3f}/{:.3f} (at time {:.3f})".format(
...
def minimize_objective(
    ...
    **kwargs,
    ...
    from scipy.optimize import (  # , line_search, brute, basinhopping, minimize_scalar
        minimize,
    )
    ...
    for iteration in range(num):
        ...
        if max_iterations is None:
            ...
            result = minimize(
                objective, x0=x0, bounds=bounds, **kwargs  # maxiter=max_iterations,
