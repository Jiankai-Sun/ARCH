...
from .linear import T_MIN, acceleration_cost, check_curve, quickest_inf_accel
from .retime import (
    EPSILON,
    MultiPPoly,
    append_polys,
...
def min_two_ramp(x1, x2, v1, v2, T, a_max, v_max=INF):
    ...
    accels = [a, -a]
    p_curve = curve_from_controls(durations, accels, t0=0.0, x0=x1, v0=v1)
    # return p_curve
...
def optimize_two_ramp(x1, x2, v1, v2, a_max, v_max=INF):
    ...
    def equality(args):
        t1, t2, vp = args
        return t1 * (v1 + vp) / 2.0 + t2 * (vp + v2) / 2.0
