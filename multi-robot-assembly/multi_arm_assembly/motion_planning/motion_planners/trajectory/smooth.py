...
from .discretize import (
    derivative_discretize_curve,
    distance_discretize_curve,
    sample_discretize_curve,
    time_discretize_curve,
)
from .limits import check_spline
from .linear import T_MIN, find_lower_bound, solve_linear
...
def within_dynamical_limits(position_curve, max_v=None, max_a=None, **kwargs):
    return within_velocity_limits(
        position_curve, max_v=max_v, **kwargs
    ) and within_acceleration_limits(position_curve, max_v=max_a, **kwargs)
...
def smooth_curve(
    ...
    for iteration in irange(num):
        if elapsed_time(start_time) >= max_time:
            break
        times = curve.x
        durations = [0.0] + [t2 - t1 for t1, t2 in get_pairs(times)]  # includes start
        ...
        if intermediate:
            ...
            if (
                (local_curve is None)
                or (spline_duration(local_curve) >= current_t)
        ...
        if refit:
            ...
            if cubic:
                ...
                new_curve = solve_multi_poly(
                    new_times, new_positions, new_velocities, v_max, a_max
                )
            if (
                (new_curve is None)
                or (spline_duration(new_curve) >= spline_duration(curve))
                or not check_spline(new_curve, v_max, a_max)
            ...
            new_curve = append_polys(
                pre_curve, local_curve, post_curve
            )  # TODO: the numerics are throwing this off?
            # print(new_curve.x)
            # print(new_curve.c[...,0])
            # assert(not curve_collision_fn(new_curve, t0=None, t1=None))
            if (spline_duration(new_curve) >= spline_duration(curve)) or not check_spline(
                new_curve, v_max, a_max
            ):
                continue
        print(
...
def smooth_cubic(
    ...
    start_durations = [0] + [
        solve_linear(np.subtract(p2, p1), v_max, a_max, t_min=T_MIN, only_duration=True)
