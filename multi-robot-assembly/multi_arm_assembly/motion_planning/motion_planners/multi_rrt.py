...
from .utils import argmin, enum, get_pairs, irange, randomize, take
...
class MultiRRT(MultiTree):
    def grow(
        ...
        for i in irange(iterations):
            ...
            last = argmin(lambda n: self.distance_fn(n.config, s), nodes + new_nodes)
...
class MultiBiRRT(MultiTree):
    def grow(self, goal, iterations=50, store=ts.PATH, max_tree_size=500):
        ...
        for _ in irange(iterations):
            ...
            s = self.sample_fn()
            last1 = argmin(lambda n: self.distance_fn(n.config, s), nodes1 + new_nodes1)
            ...
            last2 = argmin(lambda n: self.distance_fn(n.config, last1.config), nodes2 + new_nodes2)
