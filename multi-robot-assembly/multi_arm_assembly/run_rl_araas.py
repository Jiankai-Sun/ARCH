...
if __name__ == '__main__':
    ...
    ap.add_argument("-par", "--use-pyaraas", action="store_true", help="path to config")
    ap.add_argument("-rs", "--run-scripted", required=False, help="play(test) network", action='store_true')
    ap.add_argument("-rsm", "--run-scripted-multiarm", required=False, help="play(test) network", action='store_true')
    ...
    araas_environment.register_envs(enable_hardware=args["enable_hardware"], use_pyaraas=args["use_pyaraas"], is_remote=True, task_name=args["task"], render=True)
    
    config_name = args['file']
    args["play"] = True
    
    print('Loading config: ', config_name)
    # stage_name = "AssemblyScriptedInsert"
    stage_name = "strut_in_elbow"
    env = gym.make("AssemblyMultiArm-v0")
    init_cfg = task_from_name(stage_name)
    env.set_mode(stage_name, init_cfg)

    if(args['run_scripted'] or args['run_scripted_multiarm']):
        env.reset()
        scripted_actions = get_scripted_actions(1, num_robots=1+int(args['run_scripted_multiarm']))
        print(scripted_actions)
        for scripted_action in scripted_actions:
            env.step(scripted_action[0])
        env.close()
    else:
