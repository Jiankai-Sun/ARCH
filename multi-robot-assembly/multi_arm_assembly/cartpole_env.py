...
class CartpoleEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: CartpoleEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.all_bodies, _ = self.cartpole.find_bodies("pole")
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        new_masses = torch.rand(len(env_ids), len(self.all_bodies)).type(torch.FloatTensor)*100

        # get the current masses of the bodies (num_assets, num_bodies)
        masses = self.cartpole.root_physx_view.get_masses().clone()
        # apply randomization on default values
        cpu_env_ids= env_ids.cpu()
        masses[cpu_env_ids[:, None], self.all_bodies] = new_masses
...
gym.register(
    id="CartpoleMine-v0",
    entry_point="multi_arm_assembly.cartpole_env:CartpoleEnv",
...
gym.register(
    ...
    kwargs={
        "env_cfg_entry_point": CartpoleEnvCfg,
        "rl_games_cfg_entry_point": "multi_arm_assembly:myenv_cfg_flow.yaml",
    },
)