...
class MyRlGamesVecEnvWrapper(IVecEnv):
    ...
    def __init__(self, env: ManagerBasedRLEnv, rl_device: str, clip_obs: float, clip_actions: float, cnn: bool = False):
    ...
    def observation_space(self) -> gym.spaces.Dict:
        ...
        policy_obs_space = self.unwrapped.single_observation_space
        assert isinstance(policy_obs_space, gymnasium.spaces.Dict)

        new_dict = {}
        for k, v in dict(policy_obs_space).items():
            new_dict[k] = gym.spaces.box.Box(v.low, v.high)

        if self.env.cnn:
            new_dict["image"] = gym.spaces.box.Box(0, 1, (self.env.image_width, self.env.image_height, 1))

        new_dict = gym.spaces.Dict(new_dict)
    ...
    def _process_obs(self, obs_dict: VecEnvObs) -> torch.Tensor | dict[str, torch.Tensor]:
        ...
        for k, v in obs_dict.items():
            obs_dict[k] = v.to(device=self._rl_device).clone()
