...
import torch 
import wandb
from multi_arm_assembly.rl_components.my_experience import ExperienceBuffer
...
class MyA2CBase(BaseAlgorithm):
    ...
    def __init__(self, base_name, params):
        ...
        if('num_flow_features' in config):
            self.num_flow_features = config['num_flow_features']
        else:
        ...
        if('flow_coef' in self.config):
            self.flow_coef = self.config['flow_coef']
    ...
    def visualize_normalizing_flow(self, epoch_num):
        if(self.model.a2c_network.flow is not None):
            samples = self.model.a2c_network.flow().sample((16384,)).cpu().numpy()
    ...
    def write_stats(self, total_time, epoch_num, step_time, play_time, update_time, a_losses, c_losses, flow_losses, entropies, kls, last_lr, lr_mul, frame, scaled_time, scaled_play_time, curr_frames):
        ...
        if(self.model.is_flow()):
            self.writer.add_scalar('losses/flow_loss', torch_ext.mean_list(flow_losses).item(), frame)
            self.visualize_normalizing_flow(epoch_num)
    ...
    def init_tensors(self):
        ...
        algo_info = {
            ...
            'num_flow_features': self.num_flow_features
        }
    ...
    def play_steps_rnn(self):
        ...
        if(self.model.is_flow()):
            flow_samples = self.model.a2c_network.flow().sample((self.dones.shape[0], ))
        ...
        for n in range(self.horizon_length):
            ...
            self.experience_buffer.update_data('rewards', n, shaped_rewards)
            if(self.model.is_flow()):
                self.experience_buffer.update_data('flow_samples', n, flow_samples)
...
class MyContinuousA2CBase(MyA2CBase):

    def __init__(self, base_name, params):
        MyA2CBase.__init__(self, base_name, params)
    ...
    def init_tensors(self):
        ...
        self.tensor_list = self.update_list + ['obses', 'states', 'dones', 'flow_samples']
    ...
    def train_epoch(self):
        ...
        for mini_ep in range(0, self.mini_epochs_num):
            ...
            for i in range(len(self.dataset)):
                a_loss, c_loss, entropy, kl, last_lr, lr_mul, cmu, csigma, b_loss, flow_loss = self.train_actor_critic(self.dataset[i])
        ...
        return batch_dict['step_time'], play_time, update_time, total_time, a_losses, c_losses, b_losses, entropies, kls, last_lr, lr_mul, flow_losses
    ...
    def prepare_dataset(self, batch_dict):
        ...
        flow_samples = batch_dict['flow_samples']
        ...
        dataset_dict['flow_samples'] = flow_samples
    ...
    def train(self):
        ...
        while True:
            ...
            step_time, play_time, update_time, sum_time, a_losses, c_losses, b_losses, entropies, kls, last_lr, lr_mul, flow_losses = self.train_epoch()
            total_time += sum_time
            ...
            if self.global_rank == 0:
                ...
                self.write_stats(total_time, epoch_num, step_time, play_time, update_time,
                                a_losses, c_losses, flow_losses, entropies, kls, last_lr, lr_mul, frame,
...
class MyA2CAgent(MyContinuousA2CBase):

    def __init__(self, base_name, params):
        MyContinuousA2CBase.__init__(self, base_name, params)
    ...
    def calc_gradients(self, input_dict):
        ...
        if self.model.is_flow():
            batch_dict['flow_samples'] = input_dict['flow_samples']         
        ...
        with torch.cuda.amp.autocast(enabled=self.mixed_precision):
            ...
            if(self.model.is_flow()):
                flow_entropy = res_dict['flow_entropy']
            else:
                flow_entropy = 0
            ...
            if(self.model.is_flow()):
                loss = loss - flow_entropy * self.flow_coef
        ...
        self.train_result = (a_loss, c_loss, entropy, \
            ...
            mu.detach(), sigma.detach(), b_loss, flow_entropy)
