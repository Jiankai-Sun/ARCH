...
from cppf2_utils.aligning import estimateSimilarityTransform
...
import math
from cppf2_utils.box import Box
from PIL import Image
from cppf2_utils.iou import IoU
...
def draw(img, imgpts, axes, color, size=3):
    ...
    color_top = (int(color[0]), int(color[1]), int(color[2]))
    for i, j in zip([0, 1, 2, 3],[1, 3, 0, 2]):
        img = cv2.line(img, tuple(imgpts[i]), tuple(imgpts[j]), color_top, size)
...
def draw_pose(img, intrinsics, rot, center, scale, color=(255, 0, 0), out_size=256, bbox=None, crop=True):
    mat = np.eye(4)
    scale_norm = np.linalg.norm(scale)
    mat[:3, :3] = rot * scale_norm
    mat[:3, -1] = center
    ...
    if crop:
        ...
        draw_image = draw(img, projected_bbox, projected_axes, color, size=3)
        return draw_image, bbox
    


def draw_cylinder_on_image(img, points, axes, color, size=3):
    num_sides = points.shape[0] // 2
    top_circle = points[:num_sides]
    bottom_circle = points[num_sides:2*num_sides]

    # Draw top and bottom circles
    for i in range(num_sides):
        next_index = (i + 1) % num_sides
        # Draw the top circle
        img = cv2.line(img, tuple(top_circle[i]), tuple(top_circle[next_index]), color, size)
        # Draw the bottom circle
        img = cv2.line(img, tuple(bottom_circle[i]), tuple(bottom_circle[next_index]), np.array(color) * 0.3, size)
        # # Draw the side lines
        if i % 6 == 0:
            img = cv2.line(img, tuple(top_circle[i]), tuple(bottom_circle[i]), np.array(color) * 0.6, size)

    # draw axes
    # img = cv2.line(img, tuple(axes[0]), tuple(axes[1]), (0, 0, 255), size)  # Z-axis in blue
    # img = cv2.line(img, tuple(axes[0]), tuple(axes[3]), (255, 0, 0), size)  # X-axis in red
    img = cv2.line(img, tuple(axes[0]), tuple(axes[2]), (0, 255, 0), size)  # Y-axis in green

    return img


def draw_pose_cylinder(img, intrinsics, rot, center, scale, color=(255, 0, 0), out_size=256, bbox=None, crop=True):
    mat = np.eye(4)
    scale_norm = np.linalg.norm(scale)
    mat[:3, :3] = rot * scale_norm
    mat[:3, -1] = center
    ...
    cylinder_3d = create_3d_cylinder(scale[1] / scale_norm, scale[0] / scale_norm / 2, scale[2] / scale_norm / 2)  # height, radius top, radius bottom
    transformed_cylinder = transform_coordinates_3d(cylinder_3d, mat)
    projected_cylinder = calculate_2d_projections(transformed_cylinder, intrinsics)
    if bbox is None:
        tl_corner = projected_cylinder.min(0)
        br_corner = projected_cylinder.max(0)
    else:
        tl_corner = np.array([bbox[0], bbox[1]])
        br_corner = np.array([bbox[2], bbox[3]])
    
    if crop:
        draw_image_cylinder = draw_cylinder_on_image(img, projected_cylinder, projected_axes, color, size=(br_corner - tl_corner).max() // 30)
        if bbox is None:
            bbox = (tl_corner[0], tl_corner[1], br_corner[0], br_corner[1])
        draw_image_cylinder = resize_crop(draw_image_cylinder, padding=1.0, out_size=out_size, bbox=bbox)[0]
        return draw_image_cylinder, bbox
    else:
        draw_image = draw_cylinder_on_image(img, projected_cylinder, projected_axes, color, size=3)
        return draw_image, bbox

def create_3d_cylinder(height, radius_top, radius_bottom, num_sides=32):
    angle = np.linspace(0, 2 * np.pi, num_sides)
    circle_top = np.vstack((radius_top*np.sin(angle), np.full(num_sides, height/2), radius_top*np.cos(angle)))
    circle_bottom = np.vstack((radius_bottom*np.sin(angle), np.full(num_sides, -height/2), radius_bottom*np.cos(angle)))
    return np.hstack((circle_top, circle_bottom))
