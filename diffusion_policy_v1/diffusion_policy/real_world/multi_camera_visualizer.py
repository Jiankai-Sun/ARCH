...
class MultiCameraVisualizer(mp.Process):
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            ...
            # print('vis_data.keys(): ', vis_data.keys())
            ...
            elif 'depth' in vis_data:
                color = []
                for depth in vis_data['depth']:
                    # Normalize depth values
                    normalized_depth = cv2.normalize(depth, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
                    # Apply a colormap for visualization (you can choose any colormap)
                    colored_depth = cv2.applyColorMap(normalized_depth, cv2.COLORMAP_JET)
                    color.append(colored_depth)
