...
from diffusion_policy.real_world.onrobot_ft_driver import OnRobotFTDriver, ForceTorqueValue
...
class RealEnv:
    def __init__(self, 
            ...
            shm_manager=None,
            enable_color=True,
            enable_depth=False, #True, # False,
            onrobot_ft_ip="192.168.7.12",
            onrobot_ft_port=49152,
            use_onrobot_ft=True,
            use_robotiq_gripper=False,
            ):
        self.enable_color = enable_color
        self.enable_depth = enable_depth
        self.use_onrobot_ft = use_onrobot_ft
        self.use_robotiq_gripper = use_robotiq_gripper
        ...
        def transform(data):
            ...
            if enable_depth:
                data['depth'] = depth_transform(data['depth'][..., None])
            return data
        ...
        realsense = MultiRealsense(
            ...
            transform=transform,
            vis_transform=vis_transform if enable_color else None,
        ...
        self.robot = robot
        if self.use_robotiq_gripper:
            self.gripper = pyRobotiqGripper.RobotiqGripper()
            self.gripper.resetActivate()
        if self.use_onrobot_ft:
            self.onRobot_ft = OnRobotFTDriver(onrobot_ft_ip, onrobot_ft_port)
    ...
    def get_obs(self) -> dict:
        ...
        # print('k: ', k, self.n_obs_steps, self.video_capture_fps, self.frequency)
        ...
        ft_data = {"force": [], "torque": []}
        if self.use_onrobot_ft:
            if self.onRobot_ft.connect():
                readings = ForceTorqueValue()
                if self.onRobot_ft.Read(readings):
                    ft_data = {
        ...
        print(ft_data)
        obs_data.update(ft_data)
        obs_data['timestamp'] = obs_align_timestamps
        # print('obs_data.keys(): ', obs_data.keys())
        # dict_keys(['camera_0', 'robot_eef_pose', 'robot_eef_pose_vel', 'robot_joint', 'robot_joint_vel', 'timestamp'])
        # print('obs_data[camera_0].shape: ', obs_data['camera_0'].shape) # (2, 640, 480, 3)
        return obs_data
