...
class AsyncVectorEnv(VectorEnv):
    ...
    def __init__(
        ...
        if self.shared_memory:
            try:
                # print('self.single_observation_space: ', self.single_observation_space)
                # self.single_observation_space:  Box(-inf, inf, (2, 256, 256, 16), float32)
                _obs_buffer = create_shared_memory(
                    self.single_observation_space, n=self.num_envs, ctx=ctx
                )
                # print('_obs_buffer: ', _obs_buffer)
                # _obs_buffer:  <SynchronizedArray wrapper for <multiprocessing.sharedctypes.c_float_Array_117440512 object at 0x7fc9d16ed740>>
