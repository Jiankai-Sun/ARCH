...
from diffusion_policy.policy.dit_hybrid_image_policy import DitHybridImagePolicy
...
class TrainDiTHybridWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # configure model
        self.model: DitHybridImagePolicy = hydra.utils.instantiate(cfg.policy)

        self.ema_model: DitHybridImagePolicy = None
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                        ...
                        for t_i in range(len(train_trajectory_id)):
                            ...
                            if shown_selected_train_trajectory_id == False and self.selected_train_trajectory_id == train_trajectory_id[
                                ...
                                with torch.no_grad():
                                    ...
                                    gt_action = batch['action'][selected_traj_index, self.n_obs_steps].float()
                                    pred_action = result['action_pred'][selected_traj_index].float()
                            ...
                            for batch_idx, (batch, val_trajectory_id) in enumerate(tepoch):
                                ...
                                for t_i in range(len(val_trajectory_id)):
                                    ...
                                    if shown_selected_val_trajectory_id == False and self.selected_val_trajectory_id == val_trajectory_id[t_i]:
                                        ...
                                        gt_action = batch['action'][selected_traj_index, self.n_obs_steps].float()  #[None]
                                        pred_action = action_dict['action_pred'][selected_traj_index].float()
                                        mse = torch.nn.functional.mse_loss(pred_action, gt_action)
                ...
                if (self.epoch % cfg.training.sample_every) == 0:
                    with torch.no_grad():
                        ...
                        print('sample gt_action: {}, pred_action: {}'.format(gt_action, pred_action))
                        print('sample diff: ', gt_action - pred_action)
                        if 'continous_action_pred' in result:
                            continous_action_pred = result['continous_action_pred'].float()
                            print('sample continuous gt_action: {}, pred action: {}'.format(obs_dict['agent_pos'][:, -1, :9], continous_action_pred))
                        # print('pred_action.shape, gt_action.shape: ', pred_action.shape, gt_action.shape)
                        mse = torch.nn.functional.mse_loss(pred_action, gt_action)
                        step_log['train_action_mse_error'] = mse.item()
                        del batch
                        del obs_dict
...
def main(cfg):
    workspace = TrainDiTHybridWorkspace(cfg)
    workspace.run()
