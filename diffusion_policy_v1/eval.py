                ...
                for batch_idx, (batch, trajectory_id) in enumerate(tepoch):
                    ...
                    for t_i in range(len(trajectory_id)):
                        ...
                        if shown_selected_val_trajectory_id == False and selected_val_trajectory_id == \
                            ...
                            gt_action = batch['action'][selected_traj_index].float()  # [None]
                            pred_action = action_dict['action_pred'][selected_traj_index].flaot()
                            mse = torch.nn.functional.mse_loss(pred_action, gt_action)
                            runner_log['val_action_o_mse_error'] = mse.item()
                            # (XYZ, RPY, gripper)
                            r_mse = torch.nn.functional.mse_loss(pred_action[:, 3:6],
                                                                 gt_action[:, 3:6])
                            runner_log['val_action_r_mse_error'] = r_mse.item()
                            t_mse = torch.nn.functional.mse_loss(pred_action[:, :3],
                                                                 gt_action[:, :3])
                            runner_log['val_action_t_mse_error'] = t_mse.item()
                            g_mse = torch.nn.functional.mse_loss(pred_action[:, 6:],
                                                                 gt_action[:, 6:])
                            runner_log['val_action_g_mse_error'] = g_mse.item()
