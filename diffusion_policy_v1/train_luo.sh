...
HYDRA_FULL_ERROR=1 python train.py --config-dir=. --config-name=image_fmb_luo_diffusion_policy_dit.yaml training.seed=42 training.device=cuda:0 hydra.run.dir='data/outputs/${now:%Y.%m.%d}/${now:%H.%M.%S}_${name}_${task_name}' # hydra.launcher=debug
# Eval
